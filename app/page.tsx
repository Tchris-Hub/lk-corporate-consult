...
type Insight = { id: string; title: string; slug: string; excerpt: string; category: string; imageUrl?: string };
    ...
    <div className="space-y-4">
      <div className="relative aspect-video overflow-hidden rounded-[1.7rem] bg-[#0a162c] shadow-2xl shadow-[#1b2a4a]/25">
        {playing ? (
          ...
          <>
            <Image src={video.poster} alt={video.title} fill sizes="(max-width: 1024px) 100vw, 58vw" className="object-cover object-[center_48%] opacity-85 transition duration-500" />
...
export default function Home() { const [menuOpen, setMenuOpen] = useState(false); const [insights, setInsights] = useState<Insight[]>([]); useEffect(() => { fetch("/api/insights").then(r => r.json()).then(setInsights).catch(() => {}); }, []); return <LazyMotion features={domAnimation}><main className="overflow-hidden bg-[#f7f4ed] text-[#1b2a4a]">
